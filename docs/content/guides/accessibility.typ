#import "../../components/index.typ": docs-chapter, docs-figure, docs-table, short-or-long
...
= #short-or-long[Basics][Basics of Accessibility] <basics>
...
Accessibility support differs based on viewer and AT. Some combinations work better than others. In our testing, #link("https://www.adobe.com/acrobat.html")[Adobe Acrobat] paired with #link("https://www.nvaccess.org/download/")[NVDA] on Windows and #link("https://support.apple.com/guide/voiceover/welcome/mac")[VoiceOver] on macOS provided the richest accessibility support. Paired with HTML export, browsers provide a more consistent baseline of accessibility when compared to PDF readers.

Only PDF and HTML export produce accessible files. Neither PNGs nor SVGs are accessible on their own. Both formats can be used in an accessible larger work by providing a @guides:accessibility:textual-representations[textual representation].
...
- Use the @figure.caption[`caption` argument of the `figure` element] to provide captions instead of adding them as text below the function call

If you want to style the default appearance of an element, do not replace it with your own custom function. Instead, use @reference:styling:set-rules[set], show-set, and @reference:styling:show-rules[show rules] to customize its appearance. Here is an example on how you can change how strong emphasis looks in your document:
...
Do not use the grid container to represent tabular data. Instead, use @table. Tables are accessible to AT users: their AT will allow them to navigate the table two-dimensionally. Tables are conserved during reflow and repurposing. When creating tables, use the @table.header and @table.footer elements to mark up the semantic roles of individual rows. The table documentation contains an @table:accessibility[accessibility section] with more information on how to make your tables accessible. Keep in mind that while AT users can access tables, it is often cumbersome to them: Tables are optimized for visual consumption. Being read the contents of a set of cells while having to recall their row and column creates additional mental load. Consider making the core takeaway of the table accessible as text or a caption elsewhere.

Likewise, if you use functions like @rotate, @scale, and @skew, take care that this transformation either has no semantic meaning or that the meaning is available to AT users elsewhere, i.e. in figure @guides:accessibility:textual-representations[alt text] or a caption.
...
You can check your design in the web app by using the built-in color blindness simulator. To use it, open the "View" menu and select the desired mode in the "Simulate color blindness" menu. You can also use other tools on the web to #link("https://daltonlens.org/colorblindness-simulator")[simulate the color perception of various color blindnesses] if you are not using our web app.
...
There are #link("https://webaim.org/resources/contrastchecker/")[tools to compare how much contrast a pair of colors has] as foreground and background. The most common one is the WCAG color contrast ratio. For a given font size, a color pair may either fail the test, get to the AA level, or reach the higher AAA level. Aim for at least AA contrast for all your color pairings.

#docs-table(
  table.header[Content][AA Ratio][AAA Ratio],

  [Large text (≥18pt, or bold and ≥14pt)],
  [3:1],
  [4.5:1],
...
To support AT use and some repurposing workflows, all elements with a semantic meaning must have a textual representation. Think about it in terms of Universal Access: If an item is not an @guides:accessibility:artifacts[artifact], it has a semantic meaning. If, however, AT cannot ingest the item, the full semantic meaning of a document is not available to AT users. Hence, to provide Universal Access, use the mechanisms built into Typst to provide alternative representations.

When you add an image, be sure to use the @image.alt[`alt` argument of the image function] to describe what's visible in the image. This alternative description (sometimes known as alt text) should describe the gist of the image: Think about how you would describe the image to a friend if you called them on the phone. To write good alternative descriptions, consider the context in which the image appears:
...
What could be a good alternative description for #link("https://commons.wikimedia.org/wiki/File:Reiher_im_Flug.jpg")[this image]? Let's consider a few examples for what _not_ to do:
...
There are resources available on the web #link("https://webaim.org/techniques/alttext/")[to learn more about writing good alternative descriptions]. The requirement to add alternative text to images applies to all image formats. Typst does not currently retain the tags of a PDF image in the compiled document, even if the PDF image file on its own was accessible.
...
This will set the @document.title[title in the document's metadata] and in the title bar of the PDF viewer or web browser. If this results in an error when using a template, consider whether your template may provide an alternative way to set the document title.
...
Never use a heading for your document title; instead, use the title element. Should you have experience with HTML, it is important to remember that the semantics of the heading element in Typst differ from HTML headings. It is encouraged to use multiple first-level headings for section headings in Typst documents. When exporting to HTML, a @title[title] will be serialized as a `h1` tag while a @heading.level[first-level heading] will be serialized as a `h2` tag. In PDF export, the title and headings will be correctly tagged based on the PDF version targeted.
...
- *Tagged PDF:* Tagged PDFs contain machine-readable data about the semantic structure of a document that AT can parse. Typst will write Tagged PDFs by default, but keep in mind that Typst can only write appropriate tags if it knows about the semantic structure of your document. Refer to the Section @guides:accessibility:maintaining-semantics[_Maintaining semantics_] to learn how to use Typst's elements to communicate semantics. To provide Universal Access, you are also responsible to provide textual representation of non-text content yourself.
...
- *Well Tagged PDF (WTPDF):* This is an industry standard that is very similar to PDF/UA-2. Like PDF/UA-2, it is not currently supported by Typst. Originally, it was drafted because both parts of the PDF/UA specification were only available at a high cost from the International Standards Organization. Hence, #link("https://pdfa.org/wtpdf/")[WTPDF] was designed so that all conforming files can also declare conformance with PDF/UA-2. By now, #link("https://pdfa.org/sponsored-standards/")[both parts of the PDF/UA specification are available free of charge], decreasing the relevance of WTPDF.
...
- *PDF/A-2a* and *PDF/A-3a:* Like the first part of PDF/A, these standards focus on creating files suitable for archival and long-term storage. Both of these standards target the newer PDF version 1.7 instead of PDF 1.4. Here too, the strictest conformance level A contains rules for accessibility. In addition to the rules in PDF/A-1a, these standards disallow the use of characters in the #link("https://en.wikipedia.org/wiki/Private_Use_Areas")[Unicode Private Use Area] whose meaning is not universally defined. Improvements over PDF/A-1 include the ability to use transparency and better reflow. When choosing between these two parts of the PDF/A standard, choose PDF/A-2a unless you need to @pdf.attach[attach] other files. Note that conformance level A has been removed from PDF/A-4 in favor of the dedicated PDF/UA standard.

The @pdf:pdf-standards[PDF reference page] contains more information about each supported standard. To enable either PDF/UA, PDF/A-2a, or PDF/A-3a, use the @pdf:command-line[appropriate flag in the CLI] or use the export dropdown and click on PDF in the web app. At the moment, you must choose between PDF/A and PDF/UA. For accessibility-focused documents, we recommend the latter.
...
- *#link("https://eur-lex.europa.eu/eli/dir/2019/882/oj")[European Accessibility Act (EAA, EU 2019/882)]*: This regulation applies to e-books, consumer banking services, e-commerce services, and more. It requires the files distributed in these applications to be accessible.
- *Americans with Disabilities Act (ADA)*: The Department of Justice will #link("https://www.ada.gov/law-and-regs/regulations/title-ii-2010-regulations/")[require public sector organizations to provide files] in accordance to WCAG under Title II of the ADA by 2026. Likewise, #link("https://www.boia.org/blog/the-robles-v.-dominos-settlement-and-why-it-matters")[private organizations can be held liable] for inaccessible digital services under the ADA and state law.
...
- *#link("https://verapdf.org")[veraPDF]:* This open-source tool can check if your PDF file conforms to the parts of the PDF/A and PDF/UA standards it declared conformance with. Use this tool if you have chosen one of these standards during export. Failures are considered bugs in Typst and should be #link("https://github.com/typst/typst/issues")[reported on GitHub].

- *#link("https://pac.pdf-accessibility.org/en")[PDF Accessibility Checker (PAC)]:* The freeware PAC checks whether your document complies with PDF/UA and WCAG rules. When you receive a hard error in the PDF/UA tab, this is considered a bug in Typst and should be #link("https://github.com/typst/typst/issues")[reported on GitHub]. Warnings in the PDF/UA and Quality tabs may either be bugs, problems in your document, or neither. Check on the #link("https://forum.typst.app/")[Forum] or on #link("https://discord.gg/2uDybryKPe")[Discord] if you are unsure. Errors and warnings in the WCAG tab indicate problems with your document.

- *#link("https://helpx.adobe.com/acrobat/using/create-verify-pdf-accessibility.html")[Accessibility Check in Adobe Acrobat Pro]:* The accessibility checker in the paid version of Adobe Acrobat checks all PDF documents for problems. Instead of checking compliance with a well-known international or industry standard, Adobe has created their own suite of tests. Because the rules behind these tests sometimes contradict international standards like PDF/UA, some of Acrobat's checks are expected to fail for Typst documents #footnote[For example, when using footnotes, the check "Lbl and LBody must be children of LI" in the "List" section is expected to fail.]. Other checks, such as the contrast check are useful and indicate problems with your document.

When doing manual checking, you can start with a checklist. If your organization places emphasis on accessibility, they will sometimes have their own list. In absence of one, you can try lists by universities such as #link("https://www.uni-bremen.de/fileadmin/user_upload/universitaet/Digitale_Transformation/Projekt_BALLON/Checklisten/2._Auflage_englisch/Checklist_for_accessible_PDF_ENG-US_ver2.pdf")[Universität Bremen (in English)] or governments such as in #link("https://a11y.canada.ca/en/pdf-accessibility-checklist/")[Canada] or by the #link("https://www.ssa.gov/accessibility/checklists/PDF_508_Compliance_Checklist.pdf")[US Social Security Administration]. Although these checklists differ in verbosity, they all cover the most essential manual checks. Many of the technical checks in them can be skipped if you choose PDF/UA-1 export in Typst. If unsure which checklist to use, choose one from an organization culturally similar to yours.
...
- Windows: Test with #link("https://www.adobe.com/acrobat.html")[Adobe Acrobat] and #link("https://www.nvaccess.org/download/")[NVDA]. NVDA is free, open-source software. A free version of Acrobat is available.
- macOS: Test with #link("https://www.adobe.com/acrobat.html")[Adobe Acrobat] and #link("https://support.apple.com/guide/voiceover/welcome/mac")[VoiceOver]. VoiceOver is the screen reader that is built into macOS and other Apple platforms.
- Linux: Test with #link("https://wiki.gnome.org/Apps/Evince/")[Evince] or #link("https://okular.kde.org/")[Okular] and #link("https://orca.gnome.org")[Orca]. All three tools are free, open-source software. However, AT support across Linux platforms lags behind what is available on Windows and macOS. Likewise, Evince and Okular have less accessibility support than Acrobat. We strongly suggest testing with Acrobat instead.
